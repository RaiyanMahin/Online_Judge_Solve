...
void baal()
{
    ...
    // cout << "   sesh    \n";
      ...
      ll c_sesh_hbe = vp[0].S.F;

      ll j_sesh_hbe = vp[1].S.F;


      string immp = "IMPOSSIBLE";


      for(ll i = 2; i < sz(vp); i++)
      {
            ll c_in = 0, j_in = 0;

            if(vp[i].F >= c_sesh_hbe)
            {
                c_in = 1;
            ...
            else if(vp[i].F >= j_sesh_hbe and !c_in)
            {
                j_in = 1;
