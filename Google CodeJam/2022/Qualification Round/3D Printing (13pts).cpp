...
void baal()
{
    ...
    ll mn1 = 1e9, mn2 = mn1, mn3 = mn1, mn4 = mn1;
    for(ll i = 0; i < 3; i++)
    {
        cin >> a[i] >> b[i] >> c[i] >> d[i];
        mn1 = min(mn1, a[i]);
    ...
    debug(mn1) debug(mn2) debug(mn3) debug(mn4)

    ll s = mn1 + mn2 + mn3 + mn4;
    if(s < 1e6)
    {
        cout << "IMPOSSIBLE\n"; return;
    }
    
   // vector < ll > aa; aa.push_back(mn1); aa.push_back(mn1);  aa.push_back(mn1);  aa.push_back(mn1); 
