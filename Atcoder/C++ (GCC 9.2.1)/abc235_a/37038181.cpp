...
void baal()
{
    ll n; cin >> n; ll a = n / 100, c = n % 10;
    string s = to_string(n); ll b = s[1] - '0';
    //debug(a) debug(b) debug(c)

    string s1 = ""; s1 += s[1];  s1 += s[2]; s1 += s[0]; ll op = toInt(s1);
    string s2 = ""; s2 += s[2];  s2 += s[0]; s2 += s[1]; ll op2 = toInt(s2);
    cout << n + op + op2;
