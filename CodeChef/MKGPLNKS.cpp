...
void baal()
{
    ll n; cin >> n; string s; cin >> s; ll c = 0;
    for(ll i = 0; i < (n - 1); i++)
    {
        if(s[i] != s[i + 1])
        {
    ...
    debug(c)
    cout << Ceil(c , 2LL) << endl; return;

    /* ll in_f = -1, in_l = -1, in_F = -1, in_L = -1;
