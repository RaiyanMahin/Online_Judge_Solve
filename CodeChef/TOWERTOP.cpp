...
//ll LIS (vector <ll> a)
//{
//    vector < ll > tmp((ll)a.size(), 0);
//    ll Length = 1;
//    tmp[0] = a[0];
//
//    for(ll i = 1; i < (ll)a.size(); i++)
//    {
//        auto it = lower_bound(tmp.begin(), tmp.begin() + Length, a[i]);
//        if(it == tmp.begin() + Length)
//            tmp[Length++] = a[i];
//        else
//            *it = a[i];
//    }
...
ll LIS(vector<ll> a)
{
    ...
    for (ll i = 0; i < n; i++) {
        ...
        // d[j - 1] <= a[i] cz non increasing
        // mane duplicate can exist
        // sudhu increasing hole d[j - 1] < a[i]
