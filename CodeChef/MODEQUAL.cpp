...
{
    mp.clear();
    ll n; sf("%lld", &n); ll a[n + 1] = {0}; ll ans = 0, mn = INT_MAX;
    ...
    for(ll i = 0; i < n; i++)
    {
        if(a[i] != mn)
        {
            ...
            ekta number re suppose 15 % (some number , 15 er choto)
            then 15 ke highest ceil(15 , 2) - 1 choto kra jae,,
            ekhn highest choto jodi min element er cehe boro hoi
            then n ta operation
            else
            n - mp[min] ta operation lagbe
