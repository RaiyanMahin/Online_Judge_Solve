...
void baal()
{
    ...
    for(ll i = 1; i <= n; i++)
    {
        ...
        dp[i][3] = x + max(dp[i - 1][1], dp[i - 1][2]);
        dp[i][1] = y + max(dp[i - 1][3], dp[i - 1][2]);
        dp[i][2] = z + max(dp[i - 1][1], dp[i - 1][3]);


    }
    debug(dp[n][3]); debug(dp[n][1]); debug(dp[n][2]);
    //debug(dp[n - 1][0]); debug(dp[n - 1][1]); debug(dp[n - 1][2]);
