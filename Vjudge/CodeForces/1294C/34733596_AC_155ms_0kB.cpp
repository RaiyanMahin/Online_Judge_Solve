...
void baal()
{
    ...
    if(even(n))
    {
        ...
        {
            pair < ll, ll > p = check(n / 2);

            if(p.F == -1 and p.S == -1) no;
            else
            {
                yes;
                cout << "2 " << p.F << ' ' << p.S << endl;
    ...
    if(odd(n))
    {
            ...
            pair < ll, ll > pp = check1(n);
            //cout << pp.F << ' ' << pp.S << endl; return;

            if(pp.F == -1 and pp.S == -1)
            {
                no; return;
            }
            ll got = 0;

            for(ll i = 3; i <= 333667; i += 2)
            {
                if(n % i == 0 and ! got)
                {


                pair < ll, ll > p = check(n / i);
                 //debug(p.F) debug(p.S)
                if(p.F != -1 and p.S != i and p.F != i and p.F != p.S)
                {
                    yes; got = 1;
                    cout << i << ' ' << p.F << ' ' << p.S << endl;
                }
