...
ll get_Xor(ll mex)
{
    if(mex % 4 == 0) return mex;
    if(mex % 4 == 1) return 1;
    if(mex % 4 == 2) return mex + 1;
    if(mex % 4 == 3) return 0;
...
{
    //boo();
    ll mex, Xor; cin >> mex >> Xor;

    ll mn_mex_xor = get_Xor(mex - 1);
    ...
    if(mn_mex_xor != Xor)
    {
        ll op = (mn_mex_xor ^ Xor);
        if(op != mex)
        {
            cout << mex + 1 << endl; return;
        }
        else
        {
            cout << mex + 2 << endl; return;
