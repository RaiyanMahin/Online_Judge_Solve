...
void baal()
{
    ll n; cin >> n; ll op1 = (n - 1) * (n - 2), op2 = (n - 1) * (n - 3), op3 = (n - 2) * (n - 3), op = n * (n - 1);
    if(n <= 2)
    {
        cout << n << endl; return;
    }
    if(odd(n))
    {
        cout << n * (n - 1) * (n - 2 ) << endl; return;
    }
    else
    {
        ll ans = 0;
        if(n % 3) ans = (n - 1) * (n) * (n - 3);
    ...
    //cout << a
//
//    ll lcm1 = op1 / __gcd(n - 1, n - 2), f_ans1 = lcm1 * (n - 3); mx = max(mx, f_ans1);
//    ll lcm2 = op2 / __gcd(n - 1, n - 3), f_ans2 = lcm2 * (n - 2);  mx = max(mx, f_ans2);
//    ll lcm3 = op3 / __gcd(n - 3, n - 2), f_ans3 = lcm3 * (n - 1);  mx = max(mx, f_ans3);
//    //ll lcm = op / __gcd(n, n - 1), f_ans = lcm * (n - 2); mx = max(mx, f_ans);


    //debug(lcm)
    //ll f_ans = lcm * (n - 1); debug(f_ans)
