...
vector < pair < ll, ll > > odd, ev;

void baal()
{
    odd.clear(); ev.clear();
    ll n; cin >> n; ll m = n; n *= 2;
    ...
    ll lagbe = (m - 1), sz1 = (ll)odd.size() / 2, sz2 = (ll)ev.size() / 2;

    for(ll i = 0; i < (ll)odd.size(); i++)
    {
        if(lagbe and sz1)
        {
            cout << odd[i].second << ' ' << odd[ odd.size() - i - 1 ].second << endl;
            lagbe--; sz1--;
        }
    }
    for(ll i = 0; i < (ll)ev.size(); i++)
    {
        if(lagbe and sz2)
        {
            cout << ev[i].second << ' ' << ev[ ev.size() - i - 1 ].second << endl;
            lagbe--; sz2--;
        }
    }
