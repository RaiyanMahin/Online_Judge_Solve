...
template < class T, T (*co)(T, T), T(ATMP)>
...
class Mptr
{
    ...
    Mptr(vector < T > &a)
    {
        ...
        for (ll i = n - 1; i > 0; i--)
        {
            SH[i] = co( SH [i << 1LL ], SH[ i << 1LL | 1LL]);
        }
    }
 
 
    void update(ll PP, T KK)
    {
        ...
        for (int i = PP ; i > 1; i >>= 1)
            SH[i >> 1] = co( SH [i], SH [ i ^ 1]);
    }
...
ll co(ll Mx, ll Mn) { return max(Mx, Mn); };
 
 
 
void baal()
{
    ll n, m; cin >> n >> m; vector < ll > a(m , 0);
    for(ll &i : a) cin >> i;
    ...
    for(ll i = 0; i < Q; i++)
    {
        ...
        ll Mx1 = abs(a - c), Mx2 = abs(b - d);
        ll op1 = min(b, d), op2 = max(b , d);
 
        ll Mp = Tt.query( op1, op2  );
