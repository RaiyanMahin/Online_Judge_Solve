...
//        visited[nd] = 1;
        ...
        for(auto u : adj[nd])
        {
            if(dis[nd] + u.S < dis[u.F])
            {
                /* mane u.F e kon node via aschi ta store krbe
 
                r dis[nd] + u.S < dis[u.F]
                eita dei u.F via ta update hobe jdi min pae...
 
                FIGURE KHATAE AKSI..
                */
                visited[u.F] = nd;
 
               // cout << " u.F = " << u.F <<  " nd = " << nd << nn;
...
//
//
//    pq.push({1,4}); pq.push({2,22}); pq.push({443,343}); pq.push({2213,43254}); pq.push({123,4234}); pq.push({1110,14234});
//    while(!pq.empty())
//    {
//        ll ff = pq.top().F, ss = pq.top().S;
//
//        cout << " ff  = " << ff << " ss = " << ss << nn;
//
//        pq.pop();
