...
void baal()
{
    ...
    ll k = n / 7, kk = k + 1; debug(k * 7) debug(kk * 7)
    k *= 7, kk *= 7;
    string s1 = to_string(k) , s2 = to_string(kk), s3 = to_string(n);
    ll m1 = o(s1 , s3), m2 = o(s2, s3);
    ll mn = min(m1, m2); debug(m1) debug(m2)
    if(mn == m2)
    {
        cout << s2 << endl; return;
    }
