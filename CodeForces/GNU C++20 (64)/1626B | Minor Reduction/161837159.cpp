...
void baal()
{
    ...
    ll got = 0, indx = -1, val = 0;
 
 
    for(ll i = 0; i < n - 1; i++)
    {
        ll op1 = s[i] - '0', op2 = s[i + 1] - '0';
        ll K = op1 + op2;
    ...
    if(got)
    {
        for(ll i = 0; i < indx; i++) cout << s[i];
        cout << val;
        for(ll i = indx + 2; i < n; i++) cout << s[i];
    ...
    ll GOT = 0;
        ...
        if(GOT)
        {
            for(ll i = 0; i < mx_i; i++) cout << s[i];
 
            cout << mx;
 
            for(ll i = mx_i + 2; i < n; i++) cout << s[i];
